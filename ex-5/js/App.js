class App {
  ...
  pushPixels() {
    ...
    this.grid.forEach((circle, index) => {
      ...
      let zoom =-0.9;
      let range = 900 * this.pixelRatio;
      let differenceX = this.mouse.x - circle.x;
      let differenceY = this.mouse.y - circle.y;
      if (distance < range) {
        //
        const l = this.map(distance, range * 2, 0, 0, Math.PI );
        const angle = Math.cos(4);
        const amt = this.map(angle, -1, 1, 0, zoom*2);
        differenceX *= amt;
        differenceY *= amt;
  ...
  draw() {
    ...
    this.grid.forEach((circle, index) => {
      ...
      circle.color = `rgba(${color.r}, ${color.g}, ${color.b},0.9)`;
      circle.draw();
    });
