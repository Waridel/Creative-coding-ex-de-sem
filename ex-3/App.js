...
function start() {
  ...
  // constante locale
  monCanvas = document.getElementById("ex-3");
  ...
  document.addEventListener("click", function (event) {
    ...
    formes.forEach((formes) => {
      if (
        (event.clientX - 60) * pixelRatio > formes.x - formes.size / 2 &&
        (event.clientX - 60) * pixelRatio < formes.x + formes.size / 2 &&
        (event.clientY - 60) * pixelRatio > formes.y - formes.size / 2 &&
        (event.clientY - 60) * pixelRatio < formes.y + formes.size / 2
      ) {
